---
title: "FSCI 2025 - API pull archiving"
...
### Get all the data from FAOSTAT API:
    ...
    # The queries are specific to the indicator
      ...
      queries_12 <- c("FBS?area=_1&area_cs=M49&element=645&item=2918&year=_1",
      ...
      # Separate queries by the number of columns they contain  
      queries_13 <- c("CAHD?area=_1&area_cs=M49&element=6120&item=7004&year=_1",
                      "CAHD?area=_1&area_cs=M49&element=6120&item=7005&year=_1")
      queries_14 <- c("GT?area=_1&area_cs=M49&element=723113&item=6518&year=_1&source=3050")
    ...
    # Run the function over list of queries, separating by the returned number of columns
      result_12 <- lapply(queries_12, FUN = getdata_fao)
      result_13 <- lapply(queries_13, FUN = getdata_fao)
      result_14 <- lapply(queries_14, FUN = getdata_fao)
        
    # Bind rows
      fao_data <- do.call(rbind, result_12)
      data2 <- do.call(rbind, result_13) 
      data3 <- do.call(rbind, result_14)
      ...
      names(data3)
        # Harmonize columns
        fao_data$Release <- ""
        fao_data$'Source Code' <- ""
        fao_data$Source <- ""
        data2$'Source Code' <- ""
        data2$Source <- ""
        data3$Release <- ""
       
      fao_data <- rbind(fao_data, data2, data3)
    
    # Clean up
      rm(result_12, result_13, result_14, data2, data3)
      
    # Pull the food price index separately because the data are monthly
    # Also need to include the flag to identify imputed values
      path <- "https://faostatservices.fao.org/api/v1/en/data/"    
      queries <- c("CP?area=_1&area_cs=M49&element=6120&item=23013&year=_1&month=_1")
      options <- "&show_codes=true&show_unit=true&show_flags=true&show_notes=true&null_values=false&datasource=PRODUCTION_AWS"
...
# SDG data re-pulled 9/22/2025 to remove 5.a.1
    # NOTE: 5.a.1 part b cannot be successfully pulled from the SDG API, the FAODATA API, or the FAOSTAT API. It has been manually downloaded.
    
    path = c("https://unstats.un.org/sdgs/UNSDGAPIV5/v1/sdg/Indicator/Data?")    
    queries = c("indicator=16.10.2",
...
### Get data from Open Budget Survey links to datasets (Update the URL as needed, list of datasets are available here: https://internationalbudget.org/open-budget-survey/download):
    url <- "https://internationalbudget.org/sites/default/files/2024-07/ibp_full_data_time_series.xlsx"
    destfile <- tempfile ()
    download.file (url, destfile, mode = 'wb')
...
rm(wbindicators, result)

###############################################
#   Save and archive API pulls                #
###############################################

save(fao_data, result_fpi, result_unicef, result_ibp,
     unemployment, underemployment, 
     vdem_data, wb_data,
     file = file.path(data_in, "APIpulls_Mar2025.RData"))
      # Note: resaved Sept 2025 without SDG data - the files left in this data file were all pulled March 2025

# Replace SDG data with data pulled Sept 2025
save(result_sdg, 
     file = file.path(data_in, "APIpull_Sep2025.RData"))

rm(fao_data, result_fpi, result_sdg, result_sdg2, result_unicef, result_ibp,
     unemployment, underemployment, vdem_data, wb_data)
...
# Calculate urban & total pop for all countries with signatories cities ####
  # Import list of cities and create data frame of cities and a data frame to identify changes from 2024 (for hard coding city extracts below)
    # 2024 list
      mufpp_cities_2024 <- readxl::read_excel(file.path(data_in, "MUFPP city data_2024.xlsx")) %>%
        as.data.frame()
      mufpp_cities_2024 <- mufpp_cities_2024[ ,2:1]
      names(mufpp_cities_2024) <- c("country", "city")
      mufpp_cities_2024 <- mufpp_cities_2024[order(mufpp_cities_2024$country, mufpp_cities_2024$city), ]
      
    # 2025 list
      mufpp_cities <- readxl::read_excel(file.path(data_in, "MUFPP signatory cities - 2025.xlsx")) %>%
        as.data.frame()
    ...
    # Identify changes in signatory cities 
      previous <- paste(mufpp_cities_2024$city, mufpp_cities_2024$country, sep = ", ") %>% as.data.frame()
    ...
    # Identify changes to list cities to add and delete from analysis
      ...
      nrow(cities_to_add) - nrow(cities_to_delete) == nrow(updated) - nrow(previous) 
      # Clean up
      rm(previous, updated, mufpp_cities_2024)
  ...
  # Data management - Fix country names to match across city list and GADM boundary data
    # Modify country names to match geodata::gadm() as needed
      gadm_cntry_names <- geodata::country_codes()$NAME %>% as.data.frame()
      mufpp_cntry_names <- unique(mufpp_cities$country) %>% as.data.frame()
      names(gadm_cntry_names) <- "country"
      names(mufpp_cntry_names) <- "country"

    # Code to identify the mismatched spellings:
    check1 <- mufpp_cntry_names %>% anti_join(gadm_cntry_names)
    check2 <- gadm_cntry_names %>% anti_join(gadm_cntry_names)
    ...
    # Make changes for mismatched spellings:
      ...
      mufpp_cities$country <- gsub("Turkiye", "Turkey", mufpp_cities$country)
      mufpp_cities$country <- gsub("Cape Verde", "Cabo Verde", mufpp_cities$country)
      
      # Check
      check3 <- mufpp_cntry_names %>% anti_join(gadm_cntry_names)

      # Clean up
      rm(gadm_cntry_names, mufpp_cntry_names, check1, check2, check3)
...
# Crop, mask & create list of SpatRasters for each country from the Landscan global data
    ...
    # Load Landscan data
    lsg_world_23 <- terra::rast(file.path(data_in, "landscan-global-2023.tif"))
  
    # Now extract the country raster
      i <- 1
      country_SpatRasters <- vector()
      while(i <= length(country_adm0s)){
        cntry_crp <- terra::crop(lsg_world_23, country_adm0s[[i]])
    ...
    save(country_populations,
     file = file.path(data_in, "country_populations_21Feb2025.RData"))
...
# Now load the GADM data, which is used to extract the city boundaries
  # Note that this is the only raw data file not included in the Github repository
      ...
      # https://gadm.org/download_world.html - current version = 4.1
  ...
  # This commented out code provides examples for investigation to make sure the cities are pulling correct polygons
    ...
    # Look for name spelling and special characters and look for duplicate names at multiple levels
        # adm2[which(adm0$COUNTRY == "Cambodia"), ]$NAME_2 %>% sort()
        ...
        # adm5[which(adm5$COUNTRY == "Cambodia"), ]$NAME_5 %>% sort()
        # adm0[grep("*Sarajevo", adm0$COUNTRY), ]$COUNTRY
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Accra", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Tegucigalpa", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
gc()
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Rourkela", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Herzliya", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Nur-Sultan", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]][1,] %>% vect()) # Astana
...
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Chimoio", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Windhoek", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "The Hague", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect() %>% aggregate())
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Agadez", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Gaya" & adm3$COUNTRY == "Niger"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Niamey", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
gc()
...
gc()
      ...
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Kazan", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]]  %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Moscow City"), ])
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Nizhny Novgorod", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Samara, Russia", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[2,] |> vect())
...
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Greater London", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Austin", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]][1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Baltimore", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Boston", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Chicago", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Hamilton" & adm2$NAME_1 == "Ohio"), ]) #Cincinnati
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Columbus, Ohio", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
        x <- osmdata::getbb(place_name = "Los Angeles", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[1]] %>% 
          st_cast("POLYGON") %>% 
          st_cast("LINESTRING") %>% 
          st_cast("POLYGON")
      mufpp_cities_vect <- c(mufpp_cities_vect, x[1, ] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Miami", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[1]][1,] %>% vect())
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Minneapolis", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[1]] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "New Haven", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "New Port Richey", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "New York", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]][1,] %>% vect())
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Pittsburgh", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
        x <- osmdata::getbb(place_name = "San Francisco", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[[2]] %>% 
          st_cast("POLYGON") %>% 
          st_cast("LINESTRING") %>% 
          st_cast("POLYGON")
      mufpp_cities_vect <- c(mufpp_cities_vect, x[2, ] %>% vect())
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Washington DC", format_out = "sf_polygon", featuretype = "city", silent = FALSE) %>% vect())
...
gc()
  ...
  # Cities added in 2025

      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Sarajevo" & adm2$COUNTRY == "Bosnia and Herzegovina"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Phnom Penh" & adm1$COUNTRY == "Cambodia"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Vaughan" & adm3$COUNTRY == "Canada"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "San José" & adm2$COUNTRY == "Costa Rica"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Dire Dawa" & adm2$COUNTRY == "Ethiopia"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Helsinki" & adm3$COUNTRY == "Finland"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Tampere" & adm3$COUNTRY == "Finland"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm4[which(adm4$NAME_4 == "Vantaa" & adm4$COUNTRY == "Finland"), ])
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Nice" & adm3$COUNTRY == "France"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Stuttgart" & adm3$COUNTRY == "Germany"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Manado" & adm2$COUNTRY == "Indonesia"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm4[which(adm4$NAME_2 == "Muaro Jambi" & adm4$COUNTRY == "Indonesia"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm3[which(adm3$NAME_3 == "Taranto" & adm3$COUNTRY == "Italy"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Irbid" & adm2$COUNTRY == "Jordan"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Vientiane" & adm1$COUNTRY == "Laos"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Chefchaouen" & adm2$COUNTRY == "Morocco"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Christchurch" & adm2$COUNTRY == "New Zealand"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Oluyole" & adm2$COUNTRY == "Nigeria"), ])
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Kraków (City)" & adm2$COUNTRY == "Poland"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Matosinhos" & adm2$COUNTRY == "Portugal"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Beira Baixa", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Iași" & adm1$COUNTRY == "Romania"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Rufisque" & adm2$COUNTRY == "Senegal"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Ziguinchor" & adm2$COUNTRY == "Senegal"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Chiang Rai" & adm1$COUNTRY == "Thailand"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Phuket" & adm1$COUNTRY == "Thailand"), ])
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Surat Thani" & adm1$COUNTRY == "Thailand"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Udon Thani" & adm1$COUNTRY == "Thailand"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Balikesir" & adm1$COUNTRY == "Turkey"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm1[which(adm1$NAME_1 == "Konya" & adm1$COUNTRY == "Turkey"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Nilüfer"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Efeler", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Mezitli", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
      mufpp_cities_vect <- c(mufpp_cities_vect, osmdata::getbb(place_name = "Muratpaşa", format_out = "sf_polygon", featuretype = "city", silent = FALSE)[1,] %>% vect())
gc()
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Ivano-Frankivs'ka" & adm2$COUNTRY == "Ukraine"), ])
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Brovars'ka" & adm2$COUNTRY == "Ukraine"), ]) # Brovary in the MUFPP list
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Kirovohrads'kyi" & adm2$COUNTRY == "Ukraine"), ]) # Kropyvnytskyi in the MUFPP list, and this is a district not a city (no city in GADM)
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Chervonohrads'ka" & adm2$COUNTRY == "Ukraine"), ]) # Sheptytskyi in the MUFPP list
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Chervonohrads'ka" & adm2$COUNTRY == "Ukraine"), ]) # Sheptytskyi in the MUFPP list
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Zaporiz'ka" & adm2$COUNTRY == "Ukraine"), ]) # Zaporizhzhia in the MUFPP list
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$NAME_2 == "Zhytomyrs'ka" & adm2$COUNTRY == "Ukraine"), ]) # Zhytomyr in the MUFPP list
      mufpp_cities_vect <- c(mufpp_cities_vect, adm2[which(adm2$VARNAME_2 == "Lang Son" & adm2$COUNTRY == "Vietnam"), ]) 

# Save file
      mufpp_cities_vect_combined <- terra::vect(mufpp_cities_vect)
      terra::writeVector(mufpp_cities_vect_combined, file.path(data_out, "mufpp_cities_vect_Mar2025.gpkg"), overwrite = TRUE)
      # To read back in and add:
        # mufpp_cities_vect_combined <- vect(file.path(data_out, "mufpp_cities_vect.gpkg"))
        # mufpp_cities_vect <- svc(mufpp_cities_vect_combined)

  # Extract population totals per city  
    lsg_world_23 <- terra::rast(file.path(data_in, "landscan-global-2023.tif"))
    i <- 1
    all_pops <- vector()
    empty_polygons <- vector()
    
    while(i <= length(mufpp_cities_vect)){
      pops <- terra::extract(lsg_world_23, mufpp_cities_vect[[i]], fun = 'sum', na.rm = TRUE)[1,2]
      all_pops <- c(all_pops, pops)
      i <- i + 1
    }
    
    # # Diagnosis code if the population extraction gives an error, then some extractions are not finding data
    #   empty_polygons <- vector()
    #   for (i in 1:length(mufpp_cities_vect)) {
    #     if (terra::is.empty(mufpp_cities_vect[[i]])) {
    #       empty_polygons <- c(empty_polygons, i)
    #       all_pops <- c(all_pops, NA)
    #     } else {
    #       pops <- terra::extract(lsg_world_23, mufpp_cities_vect[[i]], fun = 'sum', na.rm = TRUE)
    #       if (nrow(pops) > 0) {
    #         all_pops <- c(all_pops, pops[1,2])
    #       } else {
    #         all_pops <- c(all_pops, NA)
    #       }
    #     }
    #   }
    #   view(empty_polygons)
    #   # # Now look at the records that are just above the empty records to figure out which polygon extractions are not working:
    #     view(mufpp_cities_vect[[270]])
    #     # Remove records that did not pull correctly:
    #         # Convert SpatVectorCollection to a list of SpatVectors
    #           spat_vector_list <- as.list(mufpp_cities_vect)
    #         # Remove the specified elements from the list
    #           spat_vector_list <- spat_vector_list[-c(269)]
    #         # Convert the list back to a SpatVectorCollection
    #           mufpp_cities_vect <- terra::svc(spat_vector_list)
  ...
  # Assemble values into data frame
    ...
    # Add in the total and urban populations
      mufpp_2025 <- left_join(city_populations, country_populations, by = c("cities.country" = "country"))
      save(mufpp_2025, file = file.path(data_in, "mufpp_2025_archive.RData"))
      
# Clean up
  rm(adm0, adm1, adm2, adm3, adm4, adm5, all_pops, cities_to_add, cities_to_delete, city_populations, country_populations, lsg_world_23, mufpp_cities, mufpp_cities_vect, mufpp_countries, x, pops, see, i, destfile, newnames, options, path, queries, token)
